...
export default function Carousel({
  ...
  const [actualWidth, setActualWidth] = useState(baseWidth);

  useEffect(() => {
    if (!containerRef.current) return;
    const observer = new ResizeObserver((entries) => {
      if (entries[0]?.contentRect?.width) {
        setActualWidth(Math.min(baseWidth, entries[0].contentRect.width));
      }
    });
    observer.observe(containerRef.current);
    setActualWidth(Math.min(baseWidth, containerRef.current.clientWidth));
    return () => observer.disconnect();
  }, [baseWidth]);

  const containerPadding = 16;
  const itemWidth = Math.max(0, actualWidth - containerPadding * 2);
